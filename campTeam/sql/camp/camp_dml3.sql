...
--캠핑장명 cgt.name공백('',\t,\n 등등) 일괄제거
update CAMPING_GROUNDS_TABLE set name = regexp_replace(name, '\s', '');

SELECT id,LENGTH(name),LENGTH(trim(NAME))  FROM CAMPING_GROUNDS_TABLE cgt WHERE id = 1;

SELECT LENGTH(trim('새연 글램핑 카라반 설악2호점')) FROM dual;

SELECT id,image FROM CAMPING_GROUNDS_TABLE cgt WHERE NAME = '가평무릉도원캠핑장';
