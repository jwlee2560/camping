...
import com.campTeam.webapp.dao.CampRepository2;
...
public class CampingService2 {
	 ...
	 * 개인별 캠핑 추천(3조) - 2
	 *
	 * @param campName 캠프명  ex) "열두달캠핑장"
	 *
	 * @return
	 */
	public CampEntity recommendCamp(String campName) {

		CampEntity campEntity = new CampEntity();
		List<CampReviewVO> reviewList = null;

		CampEntity2 campEntity2 = campRepo2.findByCampName(campName).get();
		campEntity = new CampEntity(campEntity2);
		...
		reviewList = campReviewRepo.findAllByCampName(campName);
		...
		// 이미지
		campDAO.getImage(campName);
		campEntity.setCampIntroImages(campDAO.getImage(campName));
