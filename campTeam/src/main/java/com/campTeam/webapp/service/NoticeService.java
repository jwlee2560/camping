...
import com.campTeam.webapp.dao.NoticeDAO;
...
public class NoticeService {
	...
	@Transactional(rollbackFor = Exception.class)
	public List<NoticeVO> selectReplysById(int noticeNum) {
		
		return noticeDAO.findByNoticeReRef(noticeNum);
	}
	
	@Transactional(readOnly = true)
	public int selectNoticesCountWithoutReplies() {
		
		return (int)noticeDAO.countByNoticeReRef(0); // (댓글 아닌)원글만 추출 : notice_re_ref = 0
	} //

	@Transactional(readOnly = true)
	public List<NoticeVO> selectNoticesByPagingWithoutReplies(int currPage, int limit) {
				
		Pageable pageable = PageRequest.of(currPage-1, limit, Sort.by(Direction.DESC, "noticeNum"));
		// return noticeDAO.findAll(pageable).getContent();
		return noticeDAO.findByNoticeReRef(0, pageable).getContent(); // (댓글 아닌)원글만 추출 : notice_re_ref = 0
	} //
	...
	// 댓글 수량 조회
	@Transactional(readOnly = true)
	public int selectNoticesCountWithReplies(int noticeNum) {
		
		return (int)noticeDAO.countByNoticeReRef(noticeNum); // 댓글의 갯수 추출 : notice_re_ref = noticeNum
	} //
