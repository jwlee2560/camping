...
window.onload = function() {
	...
	for (role_update_btn of role_update_btns) {
		...
		role_update_btn.onclick = function(e) {
			...
			axios.get(`/campTeam/member/updateRoles/${id}/roleUser/${roleUser.checked}/roleAdmin/${roleAdmin.checked}`)
	...
	for (enabledUpdateBtn of enabledUpdateBtns) {
		...
		enabledUpdateBtn.onclick = (e) => {
			...
			axios.get(`/campTeam/member/changeMemberState/${id}/${enabled}`)
				 .then(function(response) {
	...
	for (updateBtn of updateBtns) {
		...
		updateBtn.onclick = (e) => {
			...
			if (mobileCheckFlag == true) { // 유효한 휴대폰 정보라면...
				...
				axios.get(`/campTeam/member/hasFldForUpdate/${id}/mobile/${mobileFld.value}`)
				 .then(function(response) {
				...
				if (mobileDuplicatedCheckFlag == false) { // 중복된 휴대폰이 없으면...
					
					alert("휴대폰 정보 수정 전송 : " + mobileFld.value);
					
					// 변경할 회원 정보 전송
					// case-1
					// axios.get(`/campTeam/member/updateMemberByAdmin/${id}/mobile/${mobileFld.value}`)
					
					// case-2
					// post 방식
					axios({
							method : "post",
							url : '/campTeam/member/updateMemberByAdmin',
							data : {
								id : id,	
	...
	for (deleteBtn of deleteBtns) {
		...
		deleteBtn.onclick = (e) => {
			...
			if (confirm(`${id} 님의 회원 정보를 정말 삭제하시겠습니까?`) == true) {
				...
				axios.get(`/campTeam/member/deleteMemberByAdmin/${id}`)
				 .then(function(response) {
